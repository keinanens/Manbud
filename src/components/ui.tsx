...
export function CaseGrid(){return <div className="case-grid">{cases.map(c=><article className="case" key={c.name}><p className="eyebrow">{c.industry}</p><p className="stat">{c.result}</p><p className="unit">{c.unit}</p><h3>{c.name}</h3><p>{c.text}</p><Link href="/kundcase/" className="text-link">Se kundresultatet <Arrow/></Link></article>)}</div>}
